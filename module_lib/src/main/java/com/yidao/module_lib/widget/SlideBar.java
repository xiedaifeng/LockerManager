...
public class SlideBar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //单个字母的高度
        int singleHeight = (int)(height/len);
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        int singleHeight = (int)( getHeight()/letters.length);

        int topOffset = (getHeight()-(letters.length*singleHeight))/2;

        //当前选中字母的索引
        final int index = (int)((event.getY()-topOffset)/(letters.length*singleHeight) * letters.length);
        //老的索引
